...
const Cart = ({ isOpen, onClose, isMobile }) => {
  ...
  return (
    <>
      <div className={`fixed top-0 right-0 z-50 bg-white shadow-2xl flex flex-col ${isMobile ? "w-full h-full" : "w-96 h-full"}`}>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {cart.length === 0 ? (
            <div className="flex flex-col items-center justify-center mt-20 gap-4">
              ...
              <button onClick={onClose} className="px-4 py-2 bg-indigo-600 text-white rounded"> Continue Shopping </button>
            </div>
          ) : (
            cart.map(item => (
              <div key={item.id} className="flex justify-between items-center mb-2 border-b border-gray-300 pb-3">
        ...
        {cart.length > 0 && (
          <div className="p-4 border-t space-y-2">
            ...
            <button onClick={handleCheckout} className="w-full bg-green-600 text-white py-2 rounded"> Checkout </button>
            <button onClick={() => dispatch({ type: "CLEAR_CART" })} className="w-full bg-red-500 text-white py-2 rounded"> Clear Cart </button>
          </div>
        )}
      </div>
